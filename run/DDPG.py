...
with tf.name_scope('S'):
    S = tf.placeholder(tf.float32, shape=[None, 9], name='s')
with tf.name_scope('R'):
    R = tf.placeholder(tf.float32, [None, 1], name='r')
with tf.name_scope('S_'):
    S_ = tf.placeholder(tf.float32, shape=[None, 9], name='s_')
...
class Actor():
    def __init__(self, sess, action_dim, action_bound, state_dim, learning_rate, replacement):
        ...
        with tf.variable_scope('Actor'):
            #input s, output a
            a = self._build_net(S, scope='eval_net', trainable=True)
            self.a = a * 13.81
    ...
    def _build_net(self, s, scope, trainable):
        with tf.variable_scope(scope):
            # output
            net = slim.fully_connected(s, 256, activation_fn=tf.nn.elu, trainable=trainable)
            #net = slim.fully_connected(net, 256, activation_fn=tf.nn.elu, trainable=trainable)
            #net = slim.fully_connected(net, 256, activation_fn=tf.nn.elu, trainable=trainable)
            net = slim.fully_connected(net, 256, activation_fn=tf.nn.elu, trainable=trainable)
            outt = slim.fully_connected(net, 1, activation_fn=tf.nn.sigmoid, trainable=trainable)
